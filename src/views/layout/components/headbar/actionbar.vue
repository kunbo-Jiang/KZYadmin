  ...
  <div class="actionbar-contanier">
    <div class="flex-box flex_j_c-flex-end" :style="style">
      ...
      <div class="action-item">
        <el-skeleton :loading="!user.name" animated :style="{ 'margin-top': (navbar.headHeight - 40) / 2 + 'px' }">
          <template #template>
            <div class="skeleton">
          ...
          <template #default>
            <el-dropdown
              ...
              @command="selfHandle">
              <div>
                <el-avatar class="portrait margin_r-10" v-if="user.name" :src="avatar" />
                <el-avatar class="portrait margin_r-10" v-else>{{ user.name ? user.name.substr(0, 1) : 'user' }}</el-avatar>
